...
namespace EFApp.Data
{
    public partial class AdventureWorksLTContext : DbContext
    {
        ...
        public async Task<int> ProductOrderedQtyAsync(int ProductId)
        {
           using (var context = new AdventureWorksLTContext())
           {
              var orders = context.SalesOrderDetails.Where(o => o.ProductId == ProductId);
              return orders.Sum(o => o.OrderQty);
           }
        }
        public async Task<List<Address>> AddressesInACityAsync(string kaupunki, int maara)
        {
            using (var context = new AdventureWorksLTContext())
            {
                var toplist = context.Addresses.OrderBy(o => o.AddressLine1)
                    .Where(o => o.City == kaupunki).Take(maara).ToList();
        ...
        public async Task<List<string>> ProductCategoryListAsync()
        {
            ...
            using (var context = new AdventureWorksLTContext())
            {
                ...
                var pCat = context.ProductCategories.ToList();

                foreach(ProductCategory PC in pCat)
                { 
                    if (PC.ParentProductCategoryId == null)
                    {
                        string Parent = ("--" + PC.ProductCategoryId + ": " + PC.Name + "--");
                        lista.Add(Parent);
                        foreach (ProductCategory p in pCat)
                        {
                            if (p.ParentProductCategoryId != null && p.ParentProductCategoryId == PC.ProductCategoryId)
                            {
                                string Sub = (p.ProductCategoryId + ": " + p.Name);
                                lista.Add(Sub);
                            }
                        }
                    }
        ...
        public async Task<Models.CustomerInfo> GetCustomerInfoAsync(int AsiakasId, string Osoite)
        {
